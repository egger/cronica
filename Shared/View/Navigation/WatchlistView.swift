...
struct WatchlistView: View {
    ...
    @State private var scrollViewTopId = UUID()
    @State private var releasedScroll = UUID()
    @State private var upcomingScroll = UUID()
    @State private var productionScroll = UUID()
    @State private var favoriteScroll = UUID()
    @State private var watchedScroll = UUID()
    ...
    var body: some View {
        ScrollViewReader { proxy in
            ...
            .onChange(of: selectedOrder) { _ in
                ...
                switch selectedOrder {
                case .released:
                    let firstItem = items.filter( { $0.isReleased }).first
                    guard let firstItem else { return }
                    withAnimation {
                        proxy.scrollTo(firstItem.notificationID, anchor: .topLeading)
                    }
                case .upcoming:
                    let firstItem = items.filter( { $0.isUpcoming }).first
                    guard let firstItem else { return }
                    withAnimation {
                        proxy.scrollTo(firstItem.notificationID, anchor: .topLeading)
                    }
                case .production:
                    let firstItem = items.filter( { $0.isInProduction }).first
                    guard let firstItem else { return }
                    withAnimation {
                        proxy.scrollTo(firstItem.notificationID, anchor: .topLeading)
                    }
                case .watched:
                    let firstItem = items.filter( { $0.isWatched }).first
                    guard let firstItem else { return }
                    withAnimation {
                        proxy.scrollTo(firstItem.notificationID, anchor: .topLeading)
                    }
                case .favorites:
                    let firstItem = items.filter( { $0.isFavorite }).first
                    guard let firstItem else { return }
                    withAnimation {
                        proxy.scrollTo(firstItem.notificationID, anchor: .topLeading)
                    }
